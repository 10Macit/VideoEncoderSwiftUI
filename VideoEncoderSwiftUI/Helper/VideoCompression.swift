...
func compressh264VideoInBackground(videoToCompress: URL, destinationPath: URL, size: CompressionSize?, compressionTransform: CompressionTransform, compressionConfig: CompressionConfig, completionHandler: @escaping (URL)->(), progressHandler: @escaping (CGFloat) -> (), errorHandler: @escaping (Error)->()) {
    ...
    do {
        ...
        let videoCompressionProps: Dictionary<String, Any> = [
            ...
            AVVideoExpectedSourceFrameRateKey: compressionConfig.videoExpectedSourceFrameRateKey,
        ]
        ...
        // Begin Compression
        let maxDuration = CMTime(value: 20, timescale: 1)
        let duration =  avAsset.duration > maxDuration ? maxDuration : avAsset.duration
        reader.timeRange = CMTimeRangeMake(start: CMTime.zero, duration: duration)
